...
async function customerFlow(browser) {
  ...
  if (keypadNumeric.value!=='1053' || keypadNumeric.focused==='searchInput' || keypadNumeric.scrollY!==keypadInitial.scrollY || keypadNumeric.bottom!==keypadInitial.bottom || keypadNumeric.height!==keypadInitial.height) throw new Error(`テンキー入力でフォーカスまたは画面位置が動きました: ${JSON.stringify(keypadNumeric)}`);
  await page.click('#toggleKeypadMode');
  await page.waitForSelector('#keypadPanel.alpha #keypad [data-key="A"]');
  await page.click('#keypad [data-key="A"]');
  const keypadAlpha = await page.evaluate(() => ({ value:document.querySelector('#searchInput').value, mode:document.querySelector('#toggleKeypadMode').textContent, scrollY:window.scrollY, bottom:Math.round(document.querySelector('#keypadPanel').getBoundingClientRect().bottom), height:Math.round(document.querySelector('#keypadPanel').getBoundingClientRect().height) }));
  if (keypadAlpha.value!=='1053A' || keypadAlpha.mode!=='123' || keypadAlpha.scrollY!==keypadInitial.scrollY || keypadAlpha.bottom!==keypadInitial.bottom || keypadAlpha.height!==keypadInitial.height) throw new Error(`ABC切替で画面位置が動きました: ${JSON.stringify(keypadAlpha)}`);
  await page.click('#searchInput');
  if (!await page.evaluate(() => document.activeElement?.id==='searchInput' && getComputedStyle(document.querySelector('#keypadPanel')).display!=='none')) throw new Error('検索バーをタップして端末キーボード用フォーカスを取得できません');
  await page.evaluate(() => { const input=document.querySelector('#searchInput');input.blur();input.value='';input.dispatchEvent(new Event('input',{bubbles:true})); });
